...
def get_meal_recommendations_for_user(user_id):
    # Define the API endpoint and headers
    url = f"https://api-staging.foodhak.com/healthprofile-group-details/{user_id}"
    headers = {
...
# Function to count tokens with tiktoken
def count_tokens_tiktoken(text, model_name="gpt-3.5-turbo"):
    tokenizer = tiktoken.encoding_for_model(model_name)
    tokens = tokenizer.encode(text)
    return len(tokens)

# Function to generate response with Claude
def generate_response_with_claude(client, prompt, system, model="claude-3-5-sonnet@20240620", max_tokens=1024, stream=True):
    ...
    # Check if token count exceeds session limits
    if token_count > 150000:
        raise HTTPException(status_code=413, detail=f"Session limit reached: Token count is {token_count}")
    ...
    return final_response.replace("```json", "").replace("```", "").strip()

# Function to generate recommendation summary using Gemini
def generate_recommendation_summary(user_profile, product_details, vector_store_context, dynamic_meal_info):
    ...
    **Remember:**
    ...
    ### Response Format (Generate a detailed response with json format within the key **response**):
    ...
    "response": "Please provide a concise summary for **{product_name}** tailored to the user's goals and preferences. 
                                ...
                                - Include a conclusion on whether the product is suitable for the user based on the user profile and daily nutritional requirements, along with an explanation of why.
    ...
    client = initialize_claude_client(PROJECT_ID, LOCATION_CLAUDE)
    ...
    **Product Analysis:**
    ...
    """
    return generate_response_with_claude(client, prompt, system_instruction)
...
async def barcode_recommend(request: RecommendationRequest, token_valid: str = Depends(verify_token)):
    ...
    while attempt < max_attempts:
        try:
            ...
            #Add user daily nutritional requirement
            meal_recommendations = get_meal_recommendations_for_user(request.user_id)
            ...
            recommendation = generate_recommendation_summary(user_profile, product_details, vector_store_context, dynamic_meal_info)
